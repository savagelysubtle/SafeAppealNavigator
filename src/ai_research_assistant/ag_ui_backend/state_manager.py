...
class AGUIConversationState:
    ...
    def __init__(self, thread_id: str, initial_state: Optional[Dict[str, Any]] = None, initial_messages: Optional[List[AGUIMessage]] = None):
    ...
    def add_message(self, message: AGUIMessage):
        ...
        logger.debug(f"Thread {self.thread_id}: Message added: {message.id} ({message.role})")
    ...
    def patch_state(self, patch_ops: List[Dict[str, Any]]) -> bool: # AG-UI SDK uses List[Any] for delta, assume list of dicts for jsonpatch
        try:
    ...
    async def send_state_delta(self, websocket: WebSocket, patch_ops: List[Dict[str, Any]]):
...
class AGUIStateManager:
    ...
    def get_or_create_conversation(self, thread_id: str, initial_state: Optional[Dict[str, Any]] = None, initial_messages: Optional[List[AGUIMessage]] = None) -> AGUIConversationState:
        if thread_id not in self.conversations:
            self.conversations[thread_id] = AGUIConversationState(thread_id, initial_state, initial_messages)
