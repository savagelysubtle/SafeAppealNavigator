...
class APIKeyTestRequest(BaseModel):
    ...
    apiKey: str = Field(..., min_length=1, description="API key for the provider")
...
async def test_api_key(request: APIKeyTestRequest):
    ...
    try:
        ...
        try:
            ...
            msg = str(e)
            return APIKeyTestResponse(
                success=False, message=msg, provider=request.provider, model=model_name
            )
    except Exception as e:
        return APIKeyTestResponse(
            success=False, message=str(e), provider=request.provider
        )
