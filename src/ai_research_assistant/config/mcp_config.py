...
This module loads MCP server configurations and provides utilities
for agents to connect to external MCP servers based on configuration files.
"""
...
class MCPConfigLoader:
    ...
    def get_server_transport_config(self, server_name: str) -> Optional[Dict[str, Any]]:
        ...
        if "working_directory" in transport:
            working_dir = Path(transport["working_directory"])
            if not working_dir.is_absolute():
                # Resolve relative to project root (3 levels up from src/ai_research_assistant/config)
                project_root = Path(__file__).parent.parent.parent.parent
                transport["working_directory"] = str(project_root / working_dir)
