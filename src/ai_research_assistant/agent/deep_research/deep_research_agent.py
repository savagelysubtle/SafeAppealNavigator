...
async def run_parallel_pdf_research_agents(
    ...
    sheets_url: str,
    browser_factory,  # function to create (browser, context)
    max_parallel: int = 2,
) -> List[Dict[str, Any]]:
    """
    ...
    async def run_agent_for_query(query):
        async with semaphore:
            browser, context = await browser_factory()
            agent = PDFResearchAgent(browser, context)
            try:
                agent_results = await agent.run([query], sheets_url)
